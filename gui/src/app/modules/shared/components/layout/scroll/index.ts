...
export const Scroll = styled.div`
  @media (pointer: fine) {
    ...
    &::-webkit-scrollbar-button:vertical:start {
      border-radius: ${({ theme }) => `${theme.metrics.inner_radius} ${theme.metrics.inner_radius} 0 0`};
    }
  
    // border radius vertical bottom
    &::-webkit-scrollbar-button:vertical:end {
      border-radius: ${({ theme }) => `0 0 ${theme.metrics.inner_radius} ${theme.metrics.inner_radius}`};
    }
  
    // border radius horizontal left
    &::-webkit-scrollbar-button:horizontal:start {
      border-radius: ${({ theme }) => `${theme.metrics.inner_radius} 0 0 ${theme.metrics.inner_radius}`};
      border-right: 3px solid ${({ theme }) => theme.colors.background};
    }
  
    // border radius horizontal right
    &::-webkit-scrollbar-button:horizontal:end {
      border-radius: ${({ theme }) => `0 ${theme.metrics.inner_radius} ${theme.metrics.inner_radius} 0`};
